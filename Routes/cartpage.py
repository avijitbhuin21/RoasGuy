...
@router.get("/basic-cart", response_class=HTMLResponse)
async def basicCart() -> HTMLResponse:
    content = _read_html_file("basicCart.html")
    return HTMLResponse(content=content, status_code=200)


@router.get("/value-cart", response_class=HTMLResponse)
async def valueCart() -> HTMLResponse:
    content = _read_html_file("valueCart.html")
    return HTMLResponse(content=content, status_code=200)


@router.get("/business-growth-cart", response_class=HTMLResponse)
